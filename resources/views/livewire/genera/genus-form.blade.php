...
<section class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">
                {{ $genus ? 'Edit Genus' : 'New Genus' }}
            </h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Capture genus metadata and tie it to a subfamily.</p>
        </div>
        <a wire:navigate="true" href="{{ route('genera.index') }}"
    ...
    <form wire:submit.prevent="save" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div>
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Subfamily</label>
            <select wire:model.defer="subfamily_id"
                ...
                @foreach ($subfamilies as $option)
                    <option value="{{ $option->id }}">{{ $option->family->name }} / {{ $option->name }}</option>
                @endforeach
            </select>
            @error('subfamily_id')
            ...
            <div class="mt-3 space-y-2">
                @if ($image_upload)
                    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700">
                        <img src="{{ $image_upload->temporaryUrl() }}" class="h-32 w-full object-cover" alt="Genus preview">
                    </div>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Preview of the new upload.</p>
                @elseif ($image)
                    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700">
                        <img src="{{ Storage::url($image) }}" class="h-32 w-full object-cover" alt="Genus image">
                    </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit"
                ...
                {{ $genus ? 'Save changes' : 'Create genus' }}
            </button>
        </div>
    </form>
</section>
