<section class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 pb-2">Seeds</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Track packets, viability, and germination.</p>
        </div>
        <a wire:navigate href="{{ route('seeds.create', absolute: false) }}"
    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="relative w-full md:max-w-sm">
            ...
            <input type="search" wire:model.debounce.500ms="search" placeholder="Search seeds..."
    ...
    @if ($seeds->isEmpty())
        <div class="rounded-3xl border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">No seeds yet</h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Log your first seed packet to get started.</p>
            <a wire:navigate href="{{ route('seeds.create', absolute: false) }}"
    ...
    @else
        <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3" wire:loading.class="opacity-60" wire:target="search,delete,perPage">
            @foreach ($seeds as $seed)
                @php
                    $photo = $seed->photos->first();
                    $image = $photo?->image_url ?? asset('images/placeholder.png');
                    $modalId = 'deleteModal-seed-' . $seed->id;
                @endphp
                <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:border-primary-200 hover:shadow-lg dark:border-gray-700 dark:bg-gray-900">
                    <a wire:navigate href="{{ route('seeds.show', $seed) }}" class="block">
                        <div class="aspect-video overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                            <img src="{{ $image }}" alt="{{ $seed->name }}" class="h-full w-full object-cover transition duration-300 hover:scale-105">
                        </div>
                        <div class="mt-4 flex items-center justify-between">
                            <div>
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $seed->name }}</h2>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $seed->scientific_name ?? 'Unknown species' }}</p>
                            </div>
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $seed->quantity ? $seed->quantity.' pcs' : 'No qty' }}</span>
                        </div>
                        <dl class="mt-3 grid grid-cols-2 gap-3 text-xs text-gray-500 dark:text-gray-400">
                            <div>
                                <dt class="uppercase tracking-wide">Vendor</dt>
                    ...
                    <div class="mt-4 flex items-center justify-between gap-3">
                        <div class="flex items-center gap-3 text-sm font-semibold">
                            <a wire:navigate href="{{ route('seeds.show', $seed) }}"
                    ...
                    <form wire:submit.prevent="delete({{ $seed->id }})">
                        <div id="{{ $modalId }}" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    ...
                                    <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this seed packet?</p>
                                    <div class="flex justify-center items-center space-x-4">
                                        ...
                                        <button type="submit"
                                            ...
                                            wire:loading.attr="disabled" wire:target="delete({{ $seed->id }})">
                                            <span wire:loading.remove wire:target="delete({{ $seed->id }})">Yes, delete</span>
                                            <span class="flex items-center justify-center gap-2" wire:loading wire:target="delete({{ $seed->id }})">
