...
<section class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 pb-2">Subfamilies</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Bridge families and genera with curated groupings.</p>
        </div>
        <a wire:navigate="true" href="{{ route('subfamilies.create', absolute: false) }}"
    ...
    @else
        <div class="flex flex-wrap items-center gap-3">
            <div class="text-sm text-gray-600 dark:text-gray-300">
                @if ($familyName)
                    Filtering by family: <span class="font-semibold">{{ $familyName }}</span>
                @endif
            </div>
            @if ($familyName)
        ...
        <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            @foreach ($subfamilies as $subfamily)
                @php
                    $image = $subfamily->image ? Storage::url($subfamily->image) : asset('images/placeholder.png');
                    $modalId = 'deleteModal-subfamily-' . $subfamily->id;
                @endphp
                <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-gray-700 dark:bg-gray-900">
                    <a wire:navigate href="{{ route('genera.index', ['subfamily_id' => $subfamily->id]) }}" class="block">
                        <div class="aspect-video overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                            <img src="{{ $image }}" alt="{{ $subfamily->name }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-105">
                        </div>
                        <div class="mt-4 flex items-center justify-between">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ $subfamily->family->name }}</p>
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $subfamily->name }}</h2>
                            </div>
                            <span class="rounded-full bg-primary-50 px-3 py-1 text-xs font-medium text-primary-700 dark:bg-primary-500/20 dark:text-primary-200">
                                {{ $subfamily->genera_count }} genera
                            </span>
                        </div>
                        <p class="mt-3 text-sm text-gray-600 dark:text-gray-300">
                            {{ \Illuminate\Support\Str::limit($subfamily->description ?? 'No description.', 120) }}
                        </p>
                    </a>
