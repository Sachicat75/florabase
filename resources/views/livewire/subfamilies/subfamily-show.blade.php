<section class="space-y-8">
    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">Subfamily of {{ $subfamily->family->name }}</p>
                <h1 class="text-3xl font-semibold text-gray-900 dark:text-gray-100">{{ $subfamily->name }}</h1>
            </div>
    ...
    <div class="grid gap-6 md:grid-cols-3">
        <article class="md:col-span-2 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <p class="mt-2 text-sm leading-relaxed text-gray-600 dark:text-gray-300">
                {{ $subfamily->description ?: 'No description provided.' }}
            </p>
        </article>
        <article class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Genera</p>
            <p class="text-3xl font-semibold text-gray-900 dark:text-gray-100">{{ $genera->count() }}</p>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Connected plants: {{ $genera->sum('plants_count') }}</p>
            <a wire:navigate href="{{ route('genera.create', absolute: false) }}"
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Genera</h2>
        ...
        @if ($genera->isEmpty())
            <div class="rounded-3xl border border-dashed border-gray-300 bg-white/50 p-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-900/40 dark:text-gray-400">
                No genera have been linked to this subfamily.
            </div>
        @else
                ...
                @foreach ($genera as $genus)
                    <article class="flex h-full flex-col rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ $genus->name }}</h3>
                                <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ $genus->plants_count }} plants</p>
                            </div>
                            <a wire:navigate href="{{ route('genera.show', $genus) }}"
                                class="inline-flex items-center rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold text-primary-600 hover:bg-primary-100 dark:bg-primary-900/30 dark:text-primary-200">
                                Open
                            </a>
                        </div>
                        <p class="mt-3 flex-1 text-sm text-gray-600 dark:text-gray-300">
                            {{ \Illuminate\Support\Str::limit($genus->description ?: 'No description provided.', 140) }}
                        </p>
    ...
    <form wire:submit.prevent="delete({{ $subfamily->id }})">
        <!-- Main modal -->
