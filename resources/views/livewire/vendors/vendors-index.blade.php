<section class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100 pb-2">Vendors</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">Keep track of favorite nurseries and sellers.</p>
        </div>
        <a wire:navigate href="{{ route('vendors.create', absolute: false) }}"
    ...
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div class="relative w-full md:max-w-sm">
            ...
            <input type="search" wire:model.debounce.500ms="search" placeholder="Search vendors..."
        ...
        <div class="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
            ...
            <div class="flex items-center gap-2 text-primary-600 dark:text-primary-300" wire:loading.flex wire:target="search,delete,perPage">
    ...
    @if ($vendors->isEmpty())
        <div class="rounded-3xl border border-dashed border-gray-300 bg-white p-10 text-center shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">No vendors yet</h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Add trusted sellers to keep track of sourcing.</p>
            <a wire:navigate href="{{ route('vendors.create', absolute: false) }}"
        ...
        <div class="grid gap-6 sm:grid-cols-2 xl:grid-cols-3" wire:loading.class="opacity-60" wire:target="search,delete,perPage">
            @foreach ($vendors as $vendor)
                @php
                    ...
                    $modalId = 'deleteModal-vendor-' . $vendor->id;
                @endphp
                <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:border-primary-200 hover:shadow-lg dark:border-gray-700 dark:bg-gray-900">
                    <a wire:navigate href="{{ route('vendors.edit', $vendor) }}" class="block">
                        <div class="aspect-video overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                            <img src="{{ $image }}" alt="{{ $vendor->name }}" class="h-full w-full object-cover transition duration-300 hover:scale-105">
                        </div>
                        <div class="mt-4">
                            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $vendor->name }}</h2>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $vendor->location ?? 'Unknown location' }}</p>
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                {{ \Illuminate\Support\Str::limit($vendor->notes ?? 'No notes yet.', 120) }}
                            </p>
                            <dl class="mt-3 flex gap-4 text-xs text-gray-500 dark:text-gray-400">
                                <div>
                                    <dt class="uppercase tracking-wide">Plants</dt>
                                    <dd class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $vendor->plants_count }}</dd>
                                </div>
                                <div>
                                    <dt class="uppercase tracking-wide">Seeds</dt>
                                    <dd class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $vendor->seeds_count }}</dd>
                                </div>
                            </dl>
                        </div>
                    </a>
                    <div class="mt-4 flex items-center justify-between gap-3">
                        <a wire:navigate href="{{ route('vendors.edit', $vendor) }}"
                            class="text-sm font-semibold text-primary-600 hover:text-primary-500">Edit</a>
                    ...
                    <form wire:submit.prevent="delete({{ $vendor->id }})">
                        <div id="{{ $modalId }}" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    ...
                                    <p class="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this vendor?</p>
                                    <div class="flex justify-center items-center space-x-4">
                                        ...
                                        <button type="submit"
                                            ...
                                            wire:loading.attr="disabled" wire:target="delete({{ $vendor->id }})">
                                            <span wire:loading.remove wire:target="delete({{ $vendor->id }})">Yes, delete</span>
                                            <span class="flex items-center justify-center gap-2" wire:loading wire:target="delete({{ $vendor->id }})">
